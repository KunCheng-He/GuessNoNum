...
### 类型检查
```bash
npx vue-tsc --noEmit # 仅执行类型检查，不生成输出文件
```
...
### TypeScript
- 始终使用严格模式（`strict: true` 在 `tsconfig.app.json` 中）
- 启用 `noUnusedLocals`、`noUnusedParameters`、`noFallthroughCasesInSwitch`、`noUncheckedSideEffectImports`
- 为函数参数和返回值定义显式类型
...
- **枚举**：枚举名用 PascalCase，成员用 UPPER_SNAKE_CASE
